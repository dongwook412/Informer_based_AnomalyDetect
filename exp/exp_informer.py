from data.data_loader import Dataset_ETT_minute, Dataset_SWaT, Dataset_WADI, Dataset_WADI_1min, Dataset_SMAP, Dataset_KDD99
...
class Exp_Informer(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            epoch_info = "Epoch: {0}, Steps: {1} | Train Loss: {2:.7f} Vali Loss: {3:.7f} Test Loss: {4:.7f} | Time: {5}\n".format(
                epoch + 1, train_steps, train_loss, vali_loss, test_loss, str(time.time() - epoch_time))
            f = open('./results/'+setting+ "/info.txt", 'a')
            f.write(epoch_info)
            f.close()
            ##################################################
